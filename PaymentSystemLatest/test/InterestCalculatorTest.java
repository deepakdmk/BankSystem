...
public class InterestCalculatorTest {
    ...
    public void testCalculation() {
        createTransactions();
        Interest.getInterestRules().add(new InterestDTO("20010101","one",new BigDecimal("1.00")));
        StatementDTO request = createStatementRequest();
        ...
        List<InterestPeriodDTO> periodsDto =  interestCalculator.generateInterestPeriods(request.getYearMonth(),30,transaction,Interest.getInterestRules());
        Interest.getInterestRules().stream().forEach(System.out::println);
        periodsDto.stream().forEach(System.out::println);

        statement.printStatement(request.getAccountNum(), Transaction.getTransactionMap().get(request.getAccountNum()));
        assertEquals(0, transaction.get(transaction.size()));
    }
